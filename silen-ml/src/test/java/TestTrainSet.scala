...
//    train.setDiscreateNum(3)
    ...
    train2.show()
    val train4 = train2.selectData(1, 2.7, 3.8)
    train4.show()
    assert(train4.size == 6)
    assert(train4.numOfAttrs == 4)
    assert(train4.labels.length == 6)
    assert(train4.labels.distinct.length == 2)
    assert(train4.labels(4) == 1.0)
